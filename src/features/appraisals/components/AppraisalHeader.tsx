              ...
              {steps.map((step, index) => <div key={step.id} className="flex items-center space-x-3">
                ...
                scale: 1
                  ...
                  <div className="flex flex-col min-w-0 flex-1">
                    <span className={cn("text-sm font-medium truncate", currentStep === step.id ? "text-foreground" : currentStep > step.id ? "text-green-600" : "text-muted-foreground")}>
                      {step.title}
                    </span>
                  ...
                  {index < steps.length - 1 && <ChevronRight className={cn("h-4 w-4 flex-shrink-0", currentStep > step.id ? "text-green-500" : "text-muted-foreground")} />}
                </div>)}
            </div>
          </div>

          <div className="md:hidden">
            {steps.find(step => step.id === currentStep) && <div className="p-4 bg-muted/50 rounded-lg">
