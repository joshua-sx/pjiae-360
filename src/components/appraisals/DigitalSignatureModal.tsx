...
export default function DigitalSignatureModal({
  ...
  const ProgressIndicator = () => <div className="flex items-center justify-center gap-2 mb-6">
      {steps.map((step, index) => <React.Fragment key={step.id}>
          <div className="flex items-center gap-2">
            <div className={cn(`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-all`, step.completed ? 'bg-green-500 text-primary-foreground' : index === currentStep ? 'bg-primary text-primary-foreground ring-4 ring-primary/20' : 'bg-muted text-muted-foreground')}>
              {step.completed ? <Check size={16} /> : index + 1}
            </div>
            <span className={cn(`text-sm hidden sm:inline`, step.completed ? 'text-green-600' : index === currentStep ? 'text-primary font-medium' : 'text-muted-foreground')}>
              {step.label}
            </span>
          </div>
  ...
  return <>
        ...
        opacity: 0
          ...
          opacity: 0
            ...
            <div className="p-6 pt-0">
              ...
              <div className={cn(`bg-muted/50 rounded-lg border-2 border-dashed border-border p-4 sm:p-8 mb-4 relative transition-all`, isHovering && 'ring-2 ring-primary ring-offset-2 ring-offset-background')}>
                <canvas ref={canvasRef} className="w-full h-48 cursor-crosshair bg-transparent touch-none" onMouseDown={startDrawing} onMouseMove={e => debouncedDraw(e.nativeEvent)} onMouseUp={stopDrawing} onMouseLeave={() => {
                setIsHovering(false);
                stopDrawing();
              }} onMouseEnter={() => setIsHovering(true)} onTouchStart={startDrawing} onTouchMove={e => debouncedDraw(e.nativeEvent)} onTouchEnd={stopDrawing} aria-label="Signature pad – draw using mouse or touch" />
                {!hasSignature && pathLength === 0 && <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              ...
              <div className="flex items-start gap-3 mb-8">
                <Checkbox id="agreement" checked={isAgreed} onCheckedChange={checked => setIsAgreed(!!checked)} className="mt-1" />
      ...
      }} className="fixed top-4 right-4 z-[100]" role="alert" aria-live="polite">
