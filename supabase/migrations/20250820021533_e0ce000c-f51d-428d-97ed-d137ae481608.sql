-- Fix RLS Policy Issues and Add Security Definer Functions
...
-- Fix perf_query_events - add proper policies instead of no policies
CREATE POLICY "Users can insert performance events"
ON public.perf_query_events
FOR INSERT
WITH CHECK (user_id = auth.uid() AND organization_id = public.get_current_user_org_id());

CREATE POLICY "Admins can view performance events"
ON public.perf_query_events
FOR SELECT
