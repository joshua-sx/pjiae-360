...
serve(async (req) => {
  ...
  try {
    ...
    logger.info("Starting enhanced import for organization", orgName);
    logger.info("Importing", people.length, "people with auth user creation");
    ...
    const { divisionMap, departmentMap } = await databaseService.createDivisionsAndDepartments(
      people,
      organizationId
    );
    ...
    const batchResult = await databaseService.processEmployeeBatch(
      people,
      databaseContext,
      user.id
    );
