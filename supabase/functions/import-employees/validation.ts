...
export const validateEmailDomain = (email: string): EmailValidationResult => {
  ...
  if (
    SECURITY_CONFIG.ALLOWED_EMAIL_DOMAINS.length > 0 &&
  ...
  people: z
    .array(
      z.object({
        ...
        jobTitle: z.string().max(150, "Job title too long").transform(sanitizeString),
        department: z
          .string()
        ...
        role: z.string().max(50, "Role name too long").transform(sanitizeString).optional(),
      })
    )
    .min(1, "At least one employee is required")
    .max(1000, "Too many employees in single import"),
  adminInfo: z.object({
    name: z
      .string()
      ...
      .transform(sanitizeName),
    email: z.string().email("Invalid admin email").transform(sanitizeEmail),
    role: z.string().max(50, "Role too long").transform(sanitizeString),
  }),
});
...
export const securityLog = (
  ...
  logger.info(`[SECURITY-${level.toUpperCase()}]`, JSON.stringify(logEntry));
};
