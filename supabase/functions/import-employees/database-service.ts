...
export class DatabaseService {
  ...
  async findOrCreateOrganization(orgName: string): Promise<OrganizationResult> {
    try {
      ...
      logger.debug("Using organization ID", org.id);
  ...
  async createDivisionsAndDepartments(
    ...
    logger.debug("Creating divisions", divisions);
    logger.debug("Creating departments", departments);
  ...
  async findOrCreateUser(
    ...
    try {
      logger.debug(`Processing employee: ${person.email}`);
      ...
      if (newUser?.user) {
        logger.debug(`New user created: ${person.email}`);

        // Generate email verification token using secure function
        const { data: tokenData, error: tokenError } = await this.supabaseAdmin.rpc(
      ...
      if (createUserError) {
        if (
          ...
          logger.debug(`User already exists: ${person.email}`);
  ...
  async assignEmployeeRole(
    ...
    try {
      const { error: roleError } = await this.supabaseAdmin.from("user_roles").upsert(
        {
          user_id: userId,
      ...
      if (roleError) {
        ...
        logger.debug(`${role} role assigned to ${email}`);
        return { success: true };
      }
    } catch (error) {
  ...
  async assignAdminRole(
    ...
    try {
      ...
      if (roleError) {
        ...
        logger.debug(`Admin role assigned to ${email}`);
        return { success: true };
      }
    } catch (error) {
  ...
  async processEmployeeBatch(
    ...
    logger.info(`Processing ${people.length} employees in batches of ${BATCH_SIZE}`);
    ...
    for (let i = 0; i < people.length; i += BATCH_SIZE) {
      ...
      logger.debug(`Processing batch ${batchNumber}/${totalBatches} (${batch.length} employees)`);
      ...
      const batchPromises = batch.map(async (person) => {
        try {
          ...
          logger.debug(`Successfully processed: ${person.email}`);
          return {
            success: true,
            email: person.email,
      ...
      logger.info(
        `Batch ${batchNumber} completed. Current totals - Success: ${successful.length}, Failed: ${failed.length}`
      );
    }

    logger.info(
      `Import processing completed. Total successful: ${successful.length}, Total failed: ${failed.length}`
    );
